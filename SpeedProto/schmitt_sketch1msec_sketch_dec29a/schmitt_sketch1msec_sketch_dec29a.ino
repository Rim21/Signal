...
void setup() {
  ...
  digitalWrite(aPin, sensorState);
  digitalWrite(bPin, sensorState);
  digitalWrite(cPin, sensorState);
  digitalWrite(dPin, sensorState);
  digitalWrite(ePin, sensorState);
  digitalWrite(fPin, sensorState);
...
boolean buttonPressed = false;
  ...
  while(true){
    ...
    delay(1); //The algorithm specifies that the above routine be called by a timer interrupt every 
